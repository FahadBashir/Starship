...
SFApp::SFApp(std::shared_ptr<SFWindow> window) : fire(0), is_running(true), sf_window(window) {
  ...
  const int number_of_aliens1 = 3;
  for(int i=0; i<number_of_aliens1; i++) {
    // place an alien at width/number_of_aliens * i
    auto alien = make_shared<SFAsset>(SFASSET_ALIEN, sf_window);
    auto pos   = Point2((canvas_w/number_of_aliens1) * i + 110, 100.0f);
    alien->SetPosition(pos);
    aliens.push_back(alien);
  }
 ...
 const int number_of_walls1 = 3;
  for(int i=0; i<number_of_walls1; i++) {
    // place an wall at width/number_of_walls * i
    auto wall = make_shared<SFAsset>(SFASSET_WALL, sf_window);
    auto pos   = Point2((canvas_w/number_of_walls1) * i + 110, 300.0f);
    wall->SetPosition(pos);
    walls.push_back(wall);
  }

 const int number_of_walls2 = 2;
  for(int i=0; i<number_of_walls2; i++) {
    // place an wall at width/number_of_walls * i
    auto wall = make_shared<SFAsset>(SFASSET_WALL, sf_window);
    auto pos   = Point2((canvas_w/number_of_walls2) * i + 162, 100.0f);
    wall->SetPosition(pos);
    walls.push_back(wall);
  }
